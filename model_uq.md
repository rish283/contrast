...
We combine the expressive power of the Gaussian reproducing kernel Hilbert space (RKHS) and the high local resolution provided by quantum physics to create a moment based uncertainty desomposition framework, called the quantum information potential field (QIPF), for quantifying the predictive uncertainty of a neural network model. We show that the QIPF moments are significantly more precise, better calibrated and much faster than Bayesian methods for model uncertainty quantification, especially in situations involving distributional shifts in test-set.
<br />
